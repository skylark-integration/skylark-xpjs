(function() {
    ...
    window.wm = {
        ...
        focusWindow: function(windowId){const targetWindow=this._windows[windowId];if(!targetWindow||!this._windowStack.includes(windowId))return;let srcIndex=this._windowStack.indexOf(windowId);this._windowStack.splice(srcIndex,1);this._windowStack.push(windowId);this._windowStack.forEach((id,i)=>{if(this._windows[id]){this._windows[id].style.zIndex=String(i+1);this._windows[id].classList.add("inactive");if(this._taskButtons[id])this._taskButtons[id].classList.remove("active")}});targetWindow.classList.remove("inactive");const taskBtn=this._taskButtons[windowId];if(taskBtn){taskBtn.classList.add("active");if(targetWindow.classList.contains("minimized")){targetWindow.classList.remove("minimized");taskBtn.classList.remove("minimized");taskBtn.classList.remove("inactive");const animatable=targetWindow.querySelector("appanimator");const cssRoot=document.querySelector(":root");const taskSizes=taskBtn.getBoundingClientRect();const viewport=this._desktop.getBoundingClientRect();cssRoot.style.setProperty("--srcWidth",`${taskSizes.width}px`);cssRoot.style.setProperty("--srcTop",`${taskSizes.top-viewport.top}px`);cssRoot.style.setProperty("--srcLeft",`${taskSizes.left-viewport.left}px`);const oldWidth=targetWindow.getAttribute("oldWidth")||targetWindow.style.width||targetWindow.getBoundingClientRect().width;const oldTop=targetWindow.getAttribute("oldTop")||targetWindow.style.top||(targetWindow.getBoundingClientRect().top-viewport.top);const oldLeft=targetWindow.getAttribute("oldLeft")||targetWindow.style.left||(targetWindow.getBoundingClientRect().left-viewport.left);cssRoot.style.setProperty("--destWidth",`${oldWidth.includes('px')?oldWidth:oldWidth+'px'}`);cssRoot.style.setProperty("--destTop",`${oldTop.includes('px')?oldTop:oldTop+'px'}`);cssRoot.style.setProperty("--destLeft",`${oldLeft.includes('px')?oldLeft:oldLeft+'px'}`);if(animatable){animatable.classList.add("animating");setTimeout(()=>animatable.classList.remove("animating"),250)}}}},
